...
`src/caramello_api/{domain}/` (models.py, router.py, operations.py).
    ...
    for rel in relationships:
        ...
        if inner and inner not in STANDARD_TYPES and inner not in special2:
            ...
            if rel_domain and rel_domain != domain:
                ci = f"from caramello_api.{rel_domain}.models import {inner}"
...
from caramello_api.shared.auth import get_current_user
from caramello_api.{domain}.models import {domain_class}, {domain_class}Read
...
def _consolidate_models(
    ...
    third_party = sorted(
        ...
        and not i.startswith("from caramello_api")
    )
    local_imports = sorted(i for i in all_imports if i.startswith("from caramello_api"))
...
def _consolidate_routers(
    ...
    third_party = sorted(
        i
        for i in all_imports
        if not i.startswith("from uuid") and not i.startswith("from caramello_api")
    )
    local_imports = sorted(i for i in all_imports if i.startswith("from caramello_api"))
